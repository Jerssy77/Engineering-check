...
export class ProjectsService implements OnModuleInit {
    ...
    return this.data
      ...
      .map((project) => {
        ...
        return {
          ...
          experience: project.experience ?? "legacy_form",
          guidedVersion: project.guidedVersion ?? 1,
  ...
  createProject(user: SessionUser, dto: CreateProjectDto) {
    ...
    const snapshot = {
      ...
      issueSourceType: dto.issueSourceType ?? "other",
    ...
    const project = this.data.createProject({
      ...
      experience: ["civil_upgrade", "mep_upgrade"].includes(dto.projectCategory)
        ? "guided_interview"
        : "legacy_form",
      guidedVersion: ["civil_upgrade", "mep_upgrade"].includes(dto.projectCategory) ? 2 : undefined,
      status: "draft",
      createdAt: now,
      updatedAt: now
    });
